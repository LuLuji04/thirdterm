...
struct Player *copyPlayer(struct Player *player)
{
    ...
    new_board = (char **)malloc(new_player->row_cnt * sizeof(char *));
    for (int i = 0; i < new_player->row_cnt; i++)
    {
        new_board[i] = (char *)malloc(new_player->col_cnt * sizeof(char));
...
int is_valid(struct Player *player, int posx, int posy,int maximizingPlayer)
{
    ...
    for (int dir = 0; dir < 8; dir++)
    {
        ...
        if ((maximizingPlayer==1&&board[x][y] != 'o' )||(maximizingPlayer==0&&board[x][y] != 'O' ))
        {
            continue;
        }
        while (1)
        {
            x += step[dir][0];
            y += step[dir][1];
            if (x < 0 || x >= player->row_cnt || y < 0 || y >= player->col_cnt || (board[x][y] >= '1' && board[x][y] <= '9'))
            {
                break;
            }
            if ((maximizingPlayer==1&&board[x][y] == 'O')||(maximizingPlayer==0&&board[x][y]=='o' ))
...
void eat(struct Player *player,int posx,int posy,int maximizingPlayer)
{
    ...
    for (int dir = 0; dir < 8; dir++)
    {
        ...
        while (1)
        {
            ...
            if (maximizingPlayer==1&&board[x][y] == 'O')
            {
                int tx=posx;
                int ty=posy;
                while (tx!=x || ty!=y)
            ...
            else if(maximizingPlayer==0&&board[x][y]=='o' )
            {
                int tx=posx;
                int ty=posy;
                while (tx!=x || ty!=y)
...
/*int get_board(struct Player *player)
{
    int board[player->row][player->col];
...
int get_stable(struct Player *player, int current)
{
    int corner = 0;  // 角点
    int steady = 0;  // 稳定子

    int board[player->row_cnt][player->col_cnt];
    for (int i = 0;i < player->row_cnt;i++)
    {
...
int is_frontier(struct Player *player)
{
    // 遍历周围的8个方向
...
int get_frontier(struct Player *player,int current)
{
    int frontier = 0;  // 前沿子
    ...
    for (int i = 1; i <= 6; i++) 
    {
        for (int j = 1; j <= 6; j++) 
        {
            if (board[i][j] != 0 && is_frontier(i, j)) 
            {
...
int evaluate(struct Player *player,int ok_cnt)
{
    ...
    //考虑稳定子和角落权重   
    player_score = get_stable(player,1) + 8 * ok_cnt + 4 * get_frontier(player,1);
...
int alpha_beta(struct Player *player, int depth, int alpha, int beta, int maximizingPlayer)
{
    ...
    if (depth == 0)
    {
        return evaluate(player, board,ok_cnt);
    }
    if (ok_cnt == 0)
    {
        return evaluate(player, board,0);
    }
    ...
    if (maximizingPlayer)
    {
        ...
        for (int i = 0; i < ok_cnt; i++)
        {
            ...
            next_board[ok_points[i].X][ok_points[i].Y] = 'O';
