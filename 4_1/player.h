...
float evaluate(struct Player *player)
{
    ...
    distance[5] =foot+ bfs_distance(player,0,5,&a);
    //printf("%d",distance[i]);


    float ghost_score=0,star_score,superstar_score,opponent_score;
    float ghost=200 ,star=10 ,superstar=2000 ,opponent=4*player->opponent_score;
    ...
    if(player->your_status > 0)
    {
        ...
        if (player->opponent_status < player->your_score&&player->opponent_score>0)
        {
            opponent_score = opponent/distance[5];
        }
        else
        {
            opponent_score=0;
    ...
    if (maximizingPlayer==0)
    {
        for (int i = 0; i < 5; i++)
        {
            ...
            if (in(tx,ty,player)&&player->mat[tx][ty]!='#')
            {
                ...
                if(depth==2&&player->your_status > 1&&(tx==player->ghost_posx[0]&&ty==player->ghost_posy[0]||tx==player->ghost_posx[1]&&ty==player->ghost_posy[1]))
                return FLT_MAX;
                else if(depth==2&&player->your_status <= 1&&(tx==player->ghost_posx[0]&&ty==player->ghost_posy[0]||tx==player->ghost_posx[1]&&ty==player->ghost_posy[1]))
                return -FLT_MAX;
                else if(depth==2&&player->your_status - player->opponent_status > 1&&player->opponent_status >= 0&&(tx==player->opponent_posx&&ty==player->opponent_posx))
                return FLT_MAX;
                else if(player->your_status == 0&&player->opponent_status > 0&&(tx==player->opponent_posx&&ty==player->opponent_posx))
                return -FLT_MAX;
                else
                {
...
struct Point walk(struct Player *player)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        if (in(tx,ty,player)&&player->mat[tx][ty]!='#')
        {
            ...
            else if(player->your_status > 2&&(tx==player->ghost_posx[0]&&ty==player->ghost_posy[0]||tx==player->ghost_posx[1]&&ty==player->ghost_posy[1]))
            {
                point = initPoint(tx,ty);
                return point;
            }
            ...
            score=-FLT_MAX;
            else if(player->your_status - player->opponent_status > 0&&player->opponent_status >= 0&&(tx==player->opponent_posx&&ty==player->opponent_posx))
            {
                point = initPoint(tx,ty);
                return point;
            }
