...
float evaluate(struct Player *player)
{
    ...
    if(player->your_posx==6&&player->your_posy==0)
    {
        printf("(%d)\n",distance[4]);
    ...
    if(player->your_posx==6&&player->your_posy==2)
    {
        printf("(%d)\n",distance[4]);
         printf("%f %f %f %f\n",ghost_score,star_score,superstar_score,opponent_score);
         printf("%f\n", player->your_score + ghost_score+star_score+superstar_score+opponent_score);
...
struct Point walk(struct Player *player)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        if (in(tx,ty,player)&&player->mat[tx][ty]!='#')
        {
            ...
            //output(next_player);
            //printf("\n");
            if(player->your_posx==2&&player->your_posy==9)
            {
                printf("%d %d\n",tx,ty);
                printf("%f\n",score);
            } 
...
int main()
{
    ...
    struct Player *new_player = (struct Player *)malloc(sizeof(struct Player));
    new_player->row_cnt = 13;
    new_player->col_cnt = 15;
	...
	new_player->your_posx=2;
	new_player->your_posy=9;

    new_player->opponent_posx=7;
	new_player->opponent_posy=3;

    new_player->ghost_posx[0] = 1;
    new_player->ghost_posy[0] = 5;

    new_player->ghost_posx[1] = 1;
    new_player->ghost_posy[1] = 9;

    new_player->your_status=20;
    new_player->opponent_status=0;
    int distance[6];

    new_player->your_score=19;
    new_player->opponent_score=3;

    /*for (int i = 1;i < 6;i++)
    {
        distance[i] = bfs_distance(new_player,0,i);
